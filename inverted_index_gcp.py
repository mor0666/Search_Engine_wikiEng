    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                self._open_files[f_name] = open(f'/home/aspirdanielle/newbodysearch/postings_gcp/{f_name}', 'rb')
...
class InvertedIndex:  
    ...
    def add_doc(self, doc_id, tokens):
        ...
        self.term_total[(doc_id)] = self.term_total.get(doc_id, 0) + (len(tokens))
    ...
    #                 doc_id = int.from_bytes(b[i * TUPLE_SIZE:i * TUPLE_SIZE + 4], 'big')
    #                 tf = int.from_bytes(b[i * TUPLE_SIZE + 4:(i + 1) * TUPLE_SIZE], 'big')
    #                 #dl= int.from_bytes(b[i * TUPLE_SIZE + 4:(i + 1) * TUPLE_SIZE], 'big')
    #                 DL[(doc_id)]=DL.get(doc_id, 0)+tf
    #                 posting_list.append((doc_id, tf))
    #
    #             yield w, posting_list
    # DL={}
    ...
    def read_posting_list(self, w):
        with closing(MultiFileReader()) as reader:
            ...
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i * TUPLE_SIZE:i * TUPLE_SIZE + 4], 'big')
                tf = int.from_bytes(b[i * TUPLE_SIZE + 4:(i + 1) * TUPLE_SIZE], 'big')
                # self.DL[doc_id]= self.DL.get(doc_id, tf)
                #self.term_total[doc_id] = self.term_total.get(doc_id, 0) + tf
                #self.term_total[]
                posting_list.append((doc_id, tf))
            return list(self.df.keys()), posting_list
    @staticmethod
    def read_posting_list_dl(self, w):
        with closing(MultiFileReader()) as reader:
            locs = self.posting_locs[w]
            b = reader.read(locs, self.df[w] * TUPLE_SIZE)
            #print(self.df)
            ...
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i * TUPLE_SIZE:i * TUPLE_SIZE + 4], 'big')
                tf = int.from_bytes(b[i * TUPLE_SIZE + 4:(i + 1) * TUPLE_SIZE], 'big')
                # Not belong to original func.
                self.term_total[(doc_id)]=tf
                #self.term_total[]
                posting_list.append((doc_id, tf))
            return posting_list
    @staticmethod
